    ...
    <div class="row">
        <div class="col-md-6">
            ...
            @foreach( $Events as $veranstaltung)
                <table>
                    ...
                    <tr>
                        <th>Veranstalter:</th>
                        <td><a href="{{route('UserProfil',$veranstaltung->Veranstalter)}}">
                                <p> {{ $veranstaltung->Veranstalter }}</p></a></td>
                    </tr>
                    <tr>
                        <th>Datum:</th>
                ...
                @foreach($Poll as $poll)
                    ...
                    <form action={{action('LikeController@store')}}>
                        ...
                        @foreach($Option as $option)
                            ...
                            <?php if(empty($cl)){?>
                            <?php if($option->Name != ''){?>
                            <input class="radio" type="radio" value="{{$option->option}}" onclick="this.form.submit()"
                                   name="optionID">{{$option->Name}}<br>
                            <?php
                            }}?>
                        @endforeach
                ...
                <table id="poll">
                    ...
                    <td><?php if (count($Like1) == '0') {
                        } else {
                            echo count($Like1);
                        }?></td>
                    <td><?php if (count($Like2) == '0') {
                        } else {
                            echo count($Like2);
                        }?></td>
                    <td><?php if (count($Like3) == '0') {
                        } else {
                            echo count($Like3);
                        }?></td>
                    <td><?php if (count($Like4) == '0') {
                        } else {
                            echo count($Like4);
                        }?></td>
                    <td><?php if (count($Like5) == '0') {
                        } else {
                            echo count($Like5);
                        }?></td>
                    </tr>
        ...
        <div class="col-md-6">
            ...
            <form action={{action('CommentController@store')}}>
                {{csrf_field()}}
                <div><textarea size="5" rows="5" name="comment" id="comment" class="form-control"
                               placeholder="Kommentar eingeben"></textarea></div>
                <input type="hidden" name="VerID" value="<?php echo $id?>">
                <div><input type="submit" name="submit" id="submit" class="btn btn-info" value="Submit"></div>
            </form>

            @foreach($Comment as $comment)
                <a href="{{route('UserProfil',$comment->Username)}}"><h2 id="CommentHead">{{$comment->Username}}
                        am {{$comment->created_at}}</h2></a>
